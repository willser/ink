    ...
    V: PackedLayout,
{
        ...
        F: FnOnce(&Key) -> R,
    {
        ...
        // Safety: This is safe as long as we guarantee that the closure given in `f`
        //         will not store the reference to the index key before the next call
        //         to this method.
        //         Since this method is for internal use only we can guarantee that
        //         within the Mapping type.
        let index_key = unsafe { &mut *self.index_key.get() };
